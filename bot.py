...
user_sessions: dict = {}
# { user_id: { email, password, token, created_at } }
...
async def api_get_domains() -> list[str]:
...
def time_left(created_at: float) -> str:
    ...
    bar_filled = int((rem / (MAIL_EXPIRE_MIN * 60)) * 10)
...
def kb_msg_back() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[
        InlineKeyboardButton("◀️  Back to Inbox", callback_data="inbox"),
        InlineKeyboardButton("🏠  Home",           callback_data="home"),
    ]])
...
async def on_button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Member gate ──────────────────────────────────────────────────────────
    if not await is_member(ctx.bot, user.id):
        await q.answer("🔒  Please join our channel first!", show_alert=True)
        return
